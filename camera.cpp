...
void Camera::lookAt(Vec3f eye, Vec3f at, Vec3f up)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		mat[i * 4] = leftNormal[i];
		mat[i * 4 + 1] = upDir[i];
		mat[i * 4 + 2] = viewDir[i];
	}
	mat[12] = -(leftNormal * Vec3f(eye[0], eye[1], eye[2]));
	mat[13] = -(upDir * Vec3f(eye[0], eye[1], eye[2]));
	mat[14] = -(viewDir * Vec3f(eye[0], eye[1], eye[2]));
	mat[15] = 1.f;
